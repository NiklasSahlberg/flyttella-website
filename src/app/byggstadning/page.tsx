          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">
              ...
              <motion.div className="bg-white rounded-2xl shadow-lg overflow-hidden" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
                <div className="md:flex"><div className="md:w-1/3"><img src="/cleaning_lady.png" alt={locale === 'sv' ? 'Byggstädtips' : 'Construction Cleaning Tips'} className="w-full h-64 md:h-full object-cover object-[center_20%] md:object-center" /></div><div className="md:w-2/3 p-8"><div className="flex items-center mb-4"><span className="bg-gradient-to-r from-[#0F172A] to-[#10B981] textWhite px-3 py-1 rounded-full text-sm font-medium">{locale === 'sv' ? 'Byggstädning' : 'Construction Cleaning'}</span><span className="text-gray-500 text-sm ml-4">{locale === 'sv' ? '4 min läsning' : '4 min read'}</span></div><h3 className="text-2xl md:text-3xl font-bold text-[#0F172A] mb-4">{locale === 'sv' ? 'Checklista: Byggstädning efter renovering' : 'Checklist: Construction Cleaning after Renovation'}</h3><p className="text-gray-600 text-lg leading-relaxed mb-6">{locale === 'sv' ? 'Så får du bort byggdamm och rester – rum för rum, yta för yta. En enkel ordning som sparar tid.' : 'How to remove construction dust and residues – room by room, surface by surface. A simple order that saves time.'}</p><div className="flex items-center justify-end mb-4"><Link href="/blogg" className="inline-flex items-center bg-gradient-to-r from-[#0F172A] to-[#10B981] text-white px-6 py-3 rounded-full hover:opacity-90 transition-opacity font-medium group">{locale === 'sv' ? 'Läs mer' : 'Read more'}<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></Link></div></div></div>
              </motion.div>
